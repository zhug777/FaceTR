...
def seed_torch(seed=42, deterministic=True):
    seed = int(seed)
    random.seed(seed)
...
def get_scheduler(cfg, optimizer, begin_epoch):
    ...
    if cfg.TRAIN.LR_SCHEDULE == 'constant':
        scheduler = None
    elif cfg.TRAIN.LR_SCHEDULE == 'warm_up_with_cosine_lr':
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=warm_up_with_cosine_lr, last_epoch=-1 if begin_epoch == 1 else begin_epoch - 2)
    elif cfg.TRAIN.LR_SCHEDULE == 'cosine_lr':
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=cosine_lr, last_epoch=-1 if begin_epoch == 1 else begin_epoch - 2)
