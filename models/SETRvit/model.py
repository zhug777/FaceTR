...
from models.SETRvit.transformer import TransEncoder, TransConfig, ACT2FN
...
class PositionalEmbedding1D(nn.Module):
    ...
    def __init__(self, config, seq_len):
        super().__init__()
        self.pos_embedding = nn.Parameter(torch.zeros(1, seq_len, config.hidden_size))
        self.norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)

    def forward(self, x):
        """Input has shape `(batch_size, seq_len, emb_dim)`"""
        return self.norm(x + self.pos_embedding)
